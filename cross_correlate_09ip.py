...
def cross_correlate_05ip(input_stis_file):
	'''
	...
	'''
	pp = PdfPages('xd_profile_match_09ip.pdf')
	orientat = 4.17551#+45.11698363500195  #(WFPC2+STIS/CCD)
	#read in and rotate the WFC3 image
	wfpc2_img = fits.getdata('2009ip_otfr/wfpc2/hst_06359_40_wfpc2_f606w_wf_drz.fits', 1)  #F814W
	...
	xd_profile = np.sum(img[:, 200:900], axis = 1)
	...
	stis_plate_scale = 0.05071 #/arcsec/pix (Make sure this is STIS MAMA plate scale)  
	wfc3_platescale = 0.0394 #arcsec/pix (Make sure this is wfpc2 platescale)
...
#		binned_xd_profile = binned_xd_profile + xd_profile[indx::int(binning_factor)]
	...
	wfpc2_x_start = 1200 #2480, 2520
	wfpc2_x_end = 1400
	columns = np.arange(wfpc2_x_start, wfpc2_x_end, 200) #Determine which WFC3 columns to cross-correlate stis_slit_width_pix
	corr = []
	offset = []
	wfpc2_y_start = 1000 #bottom of the STIS slit
	...
	for column in columns:
		...
		ax1.axvspan(column+stis_slit_width_pix, wfpc2_x_end, color = 'k', alpha = 0.1)
        	ax1.set_yticks(np.arange(ax1.get_ylim()[0], ax1.get_ylim()[1], 50))
	        ax1.grid(color = 'w')
...
if __name__ == "__main__":
	...
	#cross_correlate_wfc3_stis_fuv('otfr_data2/ocdd030d0_flt.fits')  #718, 61
        #cross_correlate_wfc3_stis_ccd('2006gy/ocdd04010_flt.fits')
	cross_correlate_05ip('2009ip_otfr/ocdd01070_flt.fits')
